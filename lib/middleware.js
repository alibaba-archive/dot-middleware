...
module.exports = dot.middleware = function(options){
  ...
  options.render = options.render || function(str, dotPath, templatePath, callback) {

    var paths = [ path.dirname(dotPath) ];
    ...
    //       compress: (options.compress == 'auto' ? regex.compress.test(templatePath) : options.compress),
    //       yuicompress: options.yuicompress
    //     });

    //     // Store the dot import paths
    //     imports[dotPath] = tree.rules
    ...
    //         return {
    //           mtime : Date.now(),
    //           path : path.join(path.dirname(dotPath), rule.path)
    //         };
    //       });
  ...
  return function(req, res, next) {
    ...
    if (regex.handle.test(pathname)) {
      ...
      var templatePath = path.join(dest, pathname);
      var dotPath = path.join(src, (
        regex.compress.test(pathname)
        ? pathname.replace(regex.compress, '.dot')
        : pathname.replace(regex.handle, '.dot'
      )));

      log('source', dotPath);
      log('dest', templatePath);
      ...
      // Compile to templatePath
      var compile = function() {
        log('read', dotPath);

        fs.readFile(dotPath, 'utf8', function(err, str){
          if (err) { return error(err); }
          delete imports[dotPath];

          try {
            options.render(str, dotPath, templatePath, function(err, template){
              if (err) {
                dotError(err);
              ...
              mkdirp(path.dirname(templatePath), 0777, function(err){
                if (err) return error(err);

                fs.writeFile(templatePath, template, 'utf8', next);
              });
            });
          } catch (err) {
            dotError(err);
      ...
      if (!imports[dotPath]) { return compile(); }

      // Only check/recompile if it has not been done at before
      if (options.once && imports[dotPath]) { return next(); }

      // Compare mtimes
      fs.stat(dotPath, function(err, dotStats){
        if (err) { return error(err); }

        fs.stat(templatePath, function(err, templateStats){
          // template has not been compiled, compile it!
          if (err) {
            if ('ENOENT' == err.code) {
              log('not found', templatePath);
            ...
            log('modified', templatePath);

            return compile();
          } else {
            // Check if any of the dot imports were changed
            checkImports(dotPath, function(changed){
